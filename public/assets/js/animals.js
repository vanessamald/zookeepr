...
const getAnimals = (formData = {}) => {
  ...
  fetch(queryUrl)
  .then(response => {
    // checks if the ok property is true or false (this will check for any HTTP error)
    if (!response.ok) {
      return alert('Error: ' + response.statusText);
    }
    // if no errors parse the response into readable JSON format
    return response.json();
  })
  // send array of animal data to printResults() function where it generates cards for each animals
  .then(animalData => {
